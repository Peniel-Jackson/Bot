...
if not BOT_TOKEN:
    ...
    exit(1)
if not ADMIN_IDS:
    logger.error("ADMIN_IDS environment variable is required")
    exit(1)
...
# Database setup with proper path for Render
def init_database():
    # Use a proper path that works on Render
    db_path = '/tmp/payments.db' if 'RENDER' in os.environ else 'payments.db'
...
if __name__ == "__main__":
    ...
    import threading
    bot_thread = threading.Thread(target=PaymentBot().run)
